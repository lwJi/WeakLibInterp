...
This spec defines the on-disk HDF5 format of the weaklib production tables and the contract a C++ reader must satisfy to load them into host memory ready for upload to the device. It pins the group/dataset names, the logical (Fortran) shapes and their `h5ls` C-order reversal, the dtypes, the per-channel additive-offset datasets and their dimensionality, the read-order dependence (read `Names`/grid-name datasets before opening the named arrays), and the legacy fallbacks a robust reader handles. It also pins the **parallel distribution contract**: under an MPI-enabled build (see `build-integration.md`) the same loaders are transparently rank-aware — the I/O root rank reads the file and broadcasts, and every rank ends with bit-identical host arrays.
...
- The MPI distribution semantics: root-read + broadcast inside the same loader entry points, the metadata-before-arrays handshake, bit-identical replication on every rank, and the collective failure behavior. (The MPI build configuration itself — how AMReX/MPI are enabled and gated — is owned by `build-integration.md`.)
...
- `weaklib/Distributions/EOSSource/wlEOSIOModuleHDF.f90` also carries the parallel-distribution pattern: `BroadcastEquationOfStateTableParallel` (`wlEOSIOModuleHDF.f90:215-354`) is the generator-of-record for root-read + broadcast — the root rank packs a fixed integer metadata buffer (extents, `nVariables`, every index slot) that is broadcast **first** (`wlEOSIOModuleHDF.f90:263`), non-root ranks allocate from it, and then every axis/value/offset/name array is broadcast one dataset at a time. weaklib provides this for the EOS table only; this spec applies the same pattern uniformly to the opacity tables (for which weaklib has no parallel reader).
- `amrex/Src/Base/AMReX_ParallelDescriptor.H` — the communicator surface the C++ loaders use instead of raw MPI: `IOProcessorNumber()` / `IOProcessor()` select the root rank, and the templated `Bcast(T*, size_t n, int root)` distributes. Under a non-MPI AMReX build these compile to serial no-ops, so a single loader code path serves both builds.
...
For each table the reader produces, in host memory: the axis coordinate arrays (`double[]`), the per-quantity additive offset(s) (1D or 2D as documented above), and the log-stored value array(s) as a flat `double[]` in the on-disk (Fortran column-major) byte order plus their integer extents. These are exactly the inputs the per-channel interpolation specs consume and `amrex-device-interface.md` uploads to the device. Under an MPI-enabled build the reader produces these same outputs **on every rank**, bit-identically; only the I/O root rank performs file I/O, and each rank uploads its own device copy.
...
7. **MPI distribution is transparent, root-read + broadcast, bit-identical.** Under an MPI-enabled AMReX build (see `build-integration.md`) the loaders themselves are rank-aware — there is **no separate parallel entry point**. The I/O root rank (`ParallelDescriptor::IOProcessorNumber()`) opens and reads the `.h5` file exactly per requirements 1–6; every other rank receives the data by broadcast and never opens the file — only the root rank needs the file to exist and be readable. The ordering mirrors `BroadcastEquationOfStateTableParallel`: a fixed-size metadata handshake (extents, counts, offsets dimensionality, and a read-status flag) is broadcast **before** any value array, so (a) non-root ranks can allocate before receiving and (b) a root-side open/read failure is delivered to every rank as the same error — no rank may be left blocked in a broadcast (collective failure, no hang). After a successful load, every rank holds **byte-identical** host arrays, extents, offsets, and names (exact/bitwise tier — broadcasts preserve bits; no tolerance applies). In a serial or non-MPI build the same code path reduces to requirements 1–6 unchanged (the `ParallelDescriptor` calls are no-ops). Precondition: AMReX is initialized (`amrex::Initialize`) before any table load. A dataset whose element count exceeds the MPI `int` count limit (2³¹ − 1) is broadcast in chunks; every pinned production dataset is far below this.
...
5. **Rank consistency (MPI configuration only; see `build-integration.md`).** Under the MPI launcher at ≥ 2 ranks, load each table (a synthetic `.h5` fixture and, where present, the production tables) and compare a per-dataset byte digest of every loaded array — plus the extents, offsets, and names — across all ranks: all must be identical (exact tier). A 1-rank MPI run must produce arrays byte-identical to the serial build's read of the same file.
6. **Root-only file dependence and collective failure (MPI configuration only).** Confirm non-root ranks perform no HDF5 file open (mechanism is implementation freedom — e.g. an instrumented open counter), and that a root-side open failure (nonexistent path) surfaces as the same error on every rank with no rank hanging in a broadcast.
...
- The broadcast mechanics under MPI: `ParallelDescriptor::Bcast` vs. raw `MPI_Bcast` on AMReX's communicator, the chunk size for oversized datasets, how strings/names are packed (fixed-width buffers as weaklib does vs. length-prefixed), and how the metadata/status handshake is encoded — provided root-read + broadcast, metadata-before-arrays ordering, collective failure, and bit-identical replication all hold.
- Which rank is the I/O root (AMReX's `IOProcessorNumber()` is the natural choice), provided it is a single well-defined rank.

## Open questions / assumptions

- **Concrete offsets and grid extents live only in the `.h5` files (assumption, non-blocking).** The numeric `Offsets` values and the energy/η grid node coordinates are written at table-generation time and exist only inside the production tables. This spec pins the *layout* (names, shapes, dimensionality, dtype, recovery formula); the tables carry the numbers. The committed `*.h5ls` snapshots pin the structure but not the values.
...
- **Node-level host-table sharing is deferred (assumption, non-blocking).** Full per-rank replication is the contract (matching weaklib): on an 8-rank-per-node GPU machine the multi-GB host tables are duplicated 8× per node, which current node memory absorbs. An `MPI_Comm_split_type` shared-memory-window variant (one host copy per node, ranks map it read-only) is a possible future optimization; any such variant must preserve this spec's bit-identical per-rank outputs and is not part of the current contract.
- **AMReX's communicator is the communicator (assumption, non-blocking).** The loaders use `amrex::ParallelDescriptor`'s communicator; an explicit communicator-override parameter for split-communicator applications is deferred until a consumer needs it.
- **No parallel HDF5 (assumption, non-blocking).** Distribution is root-read + broadcast; MPI-IO / parallel-HDF5 collective reads are out of contract — every rank needs the identical full table, for which a broadcast is the appropriate collective, and the serial C++ HDF5 dependency stays unchanged.
