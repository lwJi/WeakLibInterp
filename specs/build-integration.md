...
- The **MPI-enabled configuration**: AMReX built with `AMReX_MPI=ON` (CPU-only, double precision unchanged) in a separate build tree, the full suite executed under the MPI launcher at ≥ 2 ranks as a second correctness-gating (ctest) build — including the rank-consistency cells of `regression-suite-design.md` and the root-read + broadcast table distribution of `table-format-and-io.md`. The default build remains MPI-free and unchanged.
...
| MPI implementation (OpenMPI / MPICH / Cray MPICH, …) | rank launch + host-side table broadcast via the `ParallelDescriptor` surface (see `table-format-and-io.md`) | required **only** for the `AMReX_MPI=ON` configuration; the default build needs no MPI |
...
- **The MPI-enabled build is supported and correctness-gating.** With AMReX configured `AMReX_MPI=ON` (CPU-only, double precision unchanged) in a separate build tree, the library and suite build against a standard MPI implementation and the **full regression suite passes under the MPI launcher at ≥ 2 ranks**, including the rank-consistency cells (`regression-suite-design.md`) and the root-read + broadcast table distribution (`table-format-and-io.md`). The default/serial build stays MPI-free and byte-for-byte unaffected; MPI is opt-in. GPU-aware MPI is **not** required: the library performs no communication inside kernels and broadcasts only host memory — each rank uploads its own device copy. A 1-rank MPI run and the serial build produce identical suite results.
...
6. **MPI build + mpirun suite passes.** With `AMReX_MPI=ON` in a separate build tree on a host with an MPI implementation, the library + suite build and the full ctest suite passes executed under the MPI launcher at 2 and at 4 ranks (launcher flags such as `--oversubscribe` on core-scarce runners are implementation freedom). A failure on any rank fails the suite. Additionally, a 1-rank MPI run agrees with the serial build's results.
...
- **CPU-only builds are the test targets; GPU builds are compile-checked only (assumption, non-blocking).** No GPU hardware is required or assumed in this environment; AMReX is built CPU-only / double precision and the suite runs on host — serially in the default build and under the MPI launcher in the MPI configuration. The CUDA/HIP backends are verified as compile checks (the qualifier macros and `ParallelFor` overloads switch to device forms and must compile), but GPU *execution* is not verified anywhere. See `amrex-device-interface.md`.
