...
| AMReX thorn options | `AMREX_DIR` — **read, never redefined**, by this thorn; it selects the one AMReX install everything links. `AMREX_ENABLE_CUDA`/`AMREX_ENABLE_HIP` are deliberately not consulted: CarpetX-style option lists leave them unset (they make the device compiler the global `CXX` directly), so the nested configure derives the backend from what the install records instead |
...
- **Configuration-consistency guard.** A mismatch between the requested backend/MPI configuration and what the install's `AMReXConfig.cmake` records (`AMReX_GPU_BACKEND`, `AMReX_MPI`) is a **configure-time hard error** naming both values — never a silent build. The thorn requests neither setting: the nested configure derives both MPI and the GPU backend from what the prefix records (an unreadable `AMReX_GPU_BACKEND` is itself a loud failure, never a silent `NONE`), so only an explicit standalone request can mismatch. The value type stays pinned: `wli::Real` is `double` regardless of the install's `AMReX_PRECISION` (the `build-integration.md` pin, unchanged here).
...
- How `build.sh` reads the backend out of the install's `AMReXConfig.cmake` and prepares the nested configure's compiler environment (scrubbing the Cactus device-`CXX`/`CXXFLAGS`, choosing the host or hip-capable compiler, `CMAKE_PREFIX_PATH` for ROCm), provided the consistency guard holds.
...
- **CUDA consumer-compiler interplay (resolved).** The first real CUDA/HIP Cactus builds (CarpetX CI option lists) surfaced the flag-forwarding detail this bullet anticipated: the Cactus environment's device-`CXX` ("nvcc -x cu" / "clang++ -x hip") leaks into the nested CMake configure and its `-x` language override breaks CMake's compiler sanity check on the test object. `build.sh` therefore scrubs `CXX`/`CXXFLAGS`/`LDFLAGS` for GPU-backend nested builds and hands CMake a host (CUDA) or hip-capable (HIP) compiler explicitly; the guard still ensures any remaining misconfiguration fails at configure time rather than at link.
