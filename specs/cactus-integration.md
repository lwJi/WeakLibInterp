...
- `amrex/Tools/CMake/AMReXInstallHelpers.cmake` — `make install` ships the entire `Tools/CMake/` module directory (including `AMReXTargetHelpers.cmake`, i.e. `setup_target_for_cuda_compilation`) into the prefix and creates the legacy `libamrex` symlink the ET thorn's `LIBRARY amrex` emission relies on. Everything the standalone build obtains from `add_subdirectory` is therefore present in an install prefix.
...
- **Installed-AMReX consumption:** configure against an AMReX install prefix via `find_package(AMReX)` as an alternative to `add_subdirectory` of a source tree.
...
- **Configuration-consistency guard.** In installed-AMReX mode, a mismatch between the requested backend/MPI configuration and what the install's `AMReXConfig.cmake` records (`AMReX_GPU_BACKEND`, `AMReX_MPI`) is a **configure-time hard error** naming both values — never a silent build. The value type stays pinned: `wli::Real` is `double` regardless of the install's `AMReX_PRECISION` (the `build-integration.md` pin, unchanged here).
...
- **Consumption-mode behavior parity.** The library built via `find_package(AMReX)` against an install prefix is behaviorally identical to the `add_subdirectory` build: the full regression suite passes with unchanged tolerance tiers (see Verification).
...
- **The standalone build stays the default and is untouched.** The sibling-source `add_subdirectory` flow remains this repo's default; installed-AMReX mode is opt-in. The default, single-precision-pin, MPI, and GPU compile-check gating builds of `build-integration.md` are byte-for-byte unaffected.
...
4. **No drift in the gating builds.** The default `build/` configure+ctest flow is unchanged by the new CMake modes (all knobs default to prior behavior).
...
- The names of the new CMake knobs (consumption-mode selector, tests-off switch) and whether installed-AMReX mode is auto-detected (prefix vs source tree at the given root) or explicitly selected.
