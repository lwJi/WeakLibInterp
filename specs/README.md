...
| [eos-interpolation](./eos-interpolation.md) | EOS single-variable 3D `(ρ,T,Yₑ)` trilinear-in-log evaluate + differentiate `_Point` contract; plus the general 2D `_Custom_Point` bilinear form on caller-pre-transformed coordinates (inversion face evaluation, consumer kernel resampling). | committed |
| [eos-inversion](./eos-inversion.md) | Recover T from `(ρ, X∈{E,P,S}, Yₑ)` via node bisection + log-linear inverse; `InitializeEOSInversion` bounds derivation/arming; integer error codes, `T=0`-on-failure, round-trip invariant. | committed |
