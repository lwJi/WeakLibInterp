...
- **NES detailed balance:** `Phi(E', E) = Phi(E, E') · exp((E − E') / (k_B·T))` (the Boltzmann exponent is the energy difference over the temperature *expressed in energy units*, `k_B·T`).
...
- `weaklib/Distributions/OpacitySource/wlOpacityInterpolationModule.f90` — cited **only for the symmetry physics**, not as an interpolation-routine source-of-truth:
  - NES detailed-balance fill of the upper energy triangle: `PhiNES(iEp, iE, l, iS, iX) = PhiNES(iE, iEp, l, iS, iX) · EXP((E(iE) − E(iEp)) / T(iX))` for `iEp > iE` (`wlOpacityInterpolationModule.f90:104-115`). In that module `T` has been pre-converted to MeV (module-internal convention), so its `(E − E')/T` is the dimensionless `(E − E')/(k_B·T_K)` of this spec's invariant — the table's temperature grid itself is in **Kelvin** (see Open questions / assumptions).
...
| 1 | T | temperature | **K** | **log** | **pre-`LOG10`** (`LogT = LOG10(T)`) |
...
- **`T` is in Kelvin — the same convention as the EOS / EmAb / Iso channels.** The table's `/ThermoState/Units` says `K` and its temperature grid spans `~1.16e9 … 1.84e12` (Kelvin; `1 MeV = 1.16045e10 K`), and thornado's production path queries it as `LOG10(T/Kelvin)` against `LOG10` of that grid verbatim (`NeutrinoOpacitiesComputationModule.F90` `UnitT = Kelvin`; `OpacityModule_TABLE.F90` `LogTs_T = LOG10(Ts_T)`). The "T and TabT in MeV" comment in weaklib's standalone wrapper (`wlOpacityInterpolationModule.f90:30`) is that dead path's *internal* pre-converted convention, overridden per the interpretation-conflict precedence (table > thornado > weaklib; `README.md`). Only the detailed-balance Boltzmann factor needs the temperature in energy units, as `k_B·T` — see Correctness requirements.
...
For the assembled NES kernel `Phi` at a fixed thermodynamic point with temperature `T` (in K, the table's unit) and neutrino energies `E(iE)`, `E(iEp)` (in MeV):

```
Phi(iEp, iE) = Phi(iE, iEp) · exp( ( E(iE) − E(iEp) ) / (k_B·T) )      for iEp > iE
```

with `k_B = 8.61733e-11 MeV/K` (weaklib's pinned value, `weaklib/TableCreator/OpacitySource/Bruenn85/Chimera/physcnst_module.f90:57`), so the exponent is dimensionless. I.e. the upper energy triangle is the lower triangle scaled by the Boltzmann factor `exp((E − E') / (k_B·T))`. This is the relation the weaklib standalone wrapper applies (`wlOpacityInterpolationModule.f90:104-115`, where `T` has been pre-converted to MeV so `k_B·T` appears there as bare `T`). A correct NES implementation must reproduce it: the upper-triangle value equals the symmetric lower-triangle value times the Boltzmann factor. Verified to the **default parity tier** (`rtol 1e-12`, `atol 1e-30`) where the lower-triangle value is exact, since the only added operations are one `exp`, one multiply, and one divide.
...
- `T` is in **Kelvin** for these channels too (per `/ThermoState/Units` and thornado's `LOG10(T/Kelvin)` query); the detailed-balance Boltzmann exponent uses `k_B·T` in MeV.
...
4. **NES detailed balance (default parity tier `1e-12`/`1e-30`).** Build a synthetic NES kernel whose lower triangle `iEp ≤ iE` is known, fill the upper triangle by `Phi(iEp, iE) = Phi(iE, iEp) · exp((E(iE) − E(iEp)) / (k_B·T))`, and assert the implementation's upper-triangle output equals the lower-triangle value times the Boltzmann factor `exp((E − E') / (k_B·T))`. Use a synthetic energy grid `E` and temperature `T` so the factor is exactly computable — the synthetic table may choose units with `k_B = 1` (energies and `k_B·T` in the same unit), stated as such; the diagonal `iEp = iE` is the fixed point (factor `= 1`).
...
- **`LogInterpolateOpacity_2D1D2D` provenance gap (assumption, explicitly non-blocking).** weaklib's standalone NES/Pair wrappers `wlInterpolateOpacity_NES` / `_Pair` (`wlOpacityInterpolationModule.f90:51-52, 149-150`) interpolate the full `(E', E)` energy grid via a routine `LogInterpolateOpacity_2D1D2D` that is **imported but not present** in the read `wlInterpolationModule.F90` source (a different revision, a generated file, or a build-time include). Its exact signature and per-axis log/linear convention are therefore unresolved. **This is off the reference consumer's critical path:** `wlOpacityInterpolationModule.f90` is never `USE`d by thornado, whose production NES/Pair path drives the **aligned** routine `LogInterpolateSingleVariable_2D2D_Custom_Aligned(_Point)` on pre-aligned `NES_AT` / `Pair_AT` tables. This spec therefore pins the **aligned** path as the source-of-truth interpolation contract, and the unresolved `2D1D2D` routine is recorded here purely as a provenance note for weaklib's standalone full-energy-grid path, which the C++ port does not need to reproduce. The Ralph loop is never blocked on this routine name: NES/Pair correctness is anchored to the aligned bilinear interpolation plus the detailed-balance / crossing-symmetry invariants, all of which are fully specified above.
- **Concrete offsets, energy/η/T grid extents, and coupling constants (assumption, non-blocking).** The 2D `Offsets[nOpacities, nMoments]` values, the energy / η / temperature node coordinates, and the weak-coupling constants `cv`, `ca` (and thus `C_i = (cv ± ca)²`, `C_ii = (cv ∓ ca)²`) live only in the production `.h5` files and the consumer's physics assembly. This spec pins the recovery contract, offset dimensionality, and the symmetry invariants; the fixture/table supplies the numbers. The exactness and symmetry checks use synthetic tables whose offsets and coupling weights the suite chooses, so they do not depend on the unknown production values.
- **Temperature units (RESOLVED: Kelvin — an interpretation conflict decided by the table > thornado > weaklib precedence, `README.md`).** An earlier revision of this spec assumed `T` in MeV, citing the `! --- T and TabT in MeV ---` comment at `wlOpacityInterpolationModule.f90:30` — tier-3 evidence from the standalone wrapper module this spec already flags as never `USE`d by thornado, whose *internal* convention pre-converts `T` to MeV. The pinned tables (tier 1) say Kelvin: `/ThermoState/Units` is `K` and the grid spans `~1.16e9 … 1.84e12` (the `1.16045…` mantissas are the `1 MeV = 1.16045e10 K` fingerprint); `/EnergyGrid/Unit` is `MeV`, `/EtaGrid/Unit` is `DIMENSIONLESS`. Thornado (tier 2) agrees: `UnitT = Kelvin`, queries as `LOG10(T/UnitT)` against `LogTs_T = LOG10(Ts_T)` with `Ts_T` the table grid verbatim. The conflict was numerically latent because bilinear-in-log interpolation is shift-invariant — a unit change shifts query and grid logs by the same constant (`log10 k_B`), leaving brackets, deltas, and the interpolant bit-identical — and only becomes observable in the detailed-balance Boltzmann factor, which therefore uses `k_B·T` (`k_B = 8.61733e-11 MeV/K`) explicitly. The consumer passes `T` in the grid's tabulated unit (K); `η` and the Boltzmann exponent are dimensionless.
