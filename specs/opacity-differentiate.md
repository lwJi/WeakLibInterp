...
- `weaklib/Distributions/Library/wlInterpolationUtilitiesModule.F90` — the two derivative cores and the raw partials that define "correct" for this leaf:
  ...
  - `dBiLineardX1` / `dBiLineardX2` (functions at `wlInterpolationUtilitiesModule.F90:254-289`) — the two raw bilinear partials: `dBiLineardX1(p00,p10,p01,p11,dX2) = Linear(p10,p11,dX2) − Linear(p00,p01,dX2)`, `dBiLineardX2(p00,p10,p01,p11,dX1) = Linear(p01,p11,dX1) − Linear(p00,p10,dX1)`. These are the ported `wli::dBiLineardX1`/`dBiLineardX2` at `src/core/wli_interp.H:159-168`.
...
- Whether `dTetraLineardX1..4` are standalone `wli::` functions (mirroring `dTriLineardX1/2/3`) or fused into the 4D core; whether `dBiLineardX1/2` are called directly or inlined.
