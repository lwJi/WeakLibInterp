...
### In-Cactus rehearsal (cactus-integration.md V#5, manual — needs network + sibling CarpetX)

- `bash cactus/config/setup-cactus-tree.sh [<cactus-dir>]` (default `../wli-cactus/cactus`, a **sibling of the repo** — deliberately outside `.build/`, which the Ralph loop wipes between iterations; the tree + bundled-AMReX prefix are multi-GB and must persist) assembles the tree: shallow-clones flesh + CactusBase + ExternalLibraries-{MPI,HDF5,zlib,yaml_cpp,AMReX}, symlinks `arrangements/CarpetX -> ../CarpetX` and `arrangements/WeakLibInterp -> cactus/thorns`, and materializes `wli-carpetx-aarch64.cfg` from the `.cfg.in` template; it prints the `make wli-carpetx-config PROMPT=no options=... THORNLIST=cactus/config/wli-carpetx.th` + `make wli-carpetx FJOBS=4` pair to run inside the tree. Always `FJOBS=` (files-per-thorn), never `make -j`: `-j` races the thorn libraries themselves and consumers compile before the bundled AMReX is installed — serial thorn builds go in sorted-thorn-name order (AMReX first, WeakLibInterp before WeakLibInterpTest), which is what the ExternalLibraries pattern relies on.
- The ET AMReX thorn builds its **bundled** AMReX (`AMREX_DIR = BUILD`, installed to the pinned `@WLI_AMREX_PREFIX@` = `../wli-cactus/opt/amrex-cactus` so the LDFLAGS rpath is stable) — the bundled version is what the sibling CarpetX checkout compiles against; the newer sibling `../amrex` breaks CarpetX (`IntVectND` API drift), so never point the Cactus config at it. Mode-switch hazard: a detect-mode configure pre-creates `configs/<cfg>/scratch/done/AMReX`, and after flipping `AMREX_DIR` to `BUILD` that stale stamp silently skips the bundled build — `rm` it before rebuilding (WeakLibInterp's own detect.sh clears its stamp automatically in build mode).
- Option-list mechanics: `F90 = none` (flesh-supported; this project never installs Fortran), `-std=gnu++20` in CXXFLAGS (AMReX ≥ 26.x C++20 requires-clauses; harmless for 25.11), `-DSIMD_DISABLE` (NSIMD omitted — it's OPTIONAL for CarpetX but `Arith/simd.hxx` needs the explicit scalar switch), serial system HDF5 with `HDF5_ENABLE_CXX=yes` / `HDF5_ENABLE_FORTRAN=no`, and explicit multiarch `*_LIB_DIRS` for yaml-cpp/zlib (ExternalLibraries `find_lib` doesn't search `/usr/lib/aarch64-linux-gnu`).
- Thorn plumbing (mirrors ExternalLibraries-AMReX exactly; all three files under `cactus/thorns/WeakLibInterp/src/`): configure-time state reaches the make phase only via MAKE_DEFINITION + `export` in `make.code.deps` (a plain `export` in detect.sh dies with the process); the `$(CCTK_TARGET) $(OBJS)...: done/$(THORN)` hook is what triggers build.sh at all; detect mode pre-creates the done stamp; build.sh resolves the repo root with `pwd -P` (the arrangement is a symlink) and reads `AMReX_MPI` out of the installed `AMReXConfig.cmake` (the ET AMReX thorn has no MPI option variable — its bundled build is implicitly MPI-on).
- `WeakLibInterp/WeakLibInterpTest` is the scratch consumer thorn: `REQUIRES WeakLibInterp AMReX MPI`, one `_Point` per kernel family at STARTUP on constant synthetic tables (constant-table exactness, `1e-12`).
