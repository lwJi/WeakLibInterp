...
Commands are shown `tools/q`-prefixed (the quiet runner from `CLAUDE.md` Build & run: output stashed to `.build/logs/<cmd>.log`, `✓` on success, full dump + exit-code passthrough on failure). `.github/workflows/ci.yml` deliberately stays raw — CI log storage is free and full logs aid post-mortems.

## Single-precision pin verification (build-integration.md §63)

`tools/q cmake -S . -B build-single -DCMAKE_BUILD_TYPE=Release -DWLI_AMREX_PRECISION=SINGLE` → `tools/q cmake --build build-single -j4` → `tools/q ctest --test-dir build-single --output-on-failure`. Separate tree required — `AMREX_USE_FLOAT` is baked into the AMReX compile, so `build-single/` is a full second AMReX build, not reusable with `build/`. `WLI_AMREX_PRECISION` defaults to `DOUBLE`; the default build is unaffected.

## MPI-enabled build + 1/2/4-rank suite (build-integration.md §61,72)

`tools/q cmake -S . -B build-mpi -DCMAKE_BUILD_TYPE=Release -DWLI_AMREX_MPI=ON` → `tools/q cmake --build build-mpi -j4` → `tools/q ctest --test-dir build-mpi --output-on-failure`.
