...
- [ ] Installed-AMReX consumption mode + configuration-consistency guard
  ...
  - notes: root `CMakeLists.txt:83-85` today supports only sibling-source `add_subdirectory(${WLI_AMREX_ROOT} ...)`; no `find_package(AMReX)` exists anywhere (grep-confirmed, hits only in specs/). Knob name is implementation freedom (spec:85) — follow the `WLI_` prefix. In installed mode, read the install's `AMReXConfig.cmake`-recorded `AMReX_GPU_BACKEND`/`AMReX_MPI`/`AMReX_PRECISION` and hard-error on mismatch with the requested `WLI_GPU_BACKEND`/`WLI_AMREX_MPI`/`WLI_AMREX_PRECISION`, naming both values. The default sibling-source path (`CMakeLists.txt:56-61` force block, `AMReX_INSTALL OFF`) must remain untouched (spec:70). Needs an AMReX installed to a prefix (AMReX's own install machinery); the CI-job item below is the standing harness for (a)/(b) — locally, install `../amrex` once into a scratch prefix to develop against.
